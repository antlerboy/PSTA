...
ASSET_VERSION='20260830-4'
LOGO=f'{PREFIX}/assets/img/psta-logo-official.jpg'
FOOTER_LOGO=f'{PREFIX}/assets/img/psta-logo-white.png'
...
def install_assets():
    ...
    if not white_source.exists():raise SystemExit('approved white PSTA logo missing')
...
def patch_pages():
    for p in ROOT.rglob('*.html'):
        ...
        s=re.sub(r'(<div\s+class=["\']footer-logo["\'][^>]*>\s*<img\b[^>]*\bsrc=["\'])[^"\']*',rf'\1{FOOTER_LOGO}',s,flags=re.I)
...
def css_fix():
    ...
    s+=mark+'''\n.brand{width:300px;min-height:100px;padding:8px 12px;background:#fff;border-radius:8px;display:flex;align-items:center;justify-content:flex-start}.brand img{display:block;width:auto;height:84px;max-width:100%;max-height:84px;object-fit:contain;object-position:left center;background:#fff}.footer-logo{display:block;width:220px;max-width:100%;margin:0 0 1.25rem;padding:0;background:transparent;border-radius:0}.footer-logo img{display:block;width:100%;height:auto;max-height:none;object-fit:contain;object-position:left center;background:transparent}.partner-logo-wrap{display:flex;align-items:center;justify-content:center;min-height:120px;margin-bottom:1rem;padding:1rem;background:#fff;border:1px solid var(--line);border-radius:12px}.partner-logo{display:block;width:auto;max-width:230px;height:auto;max-height:86px;object-fit:contain}.partner-logo-fallback{font-size:1.3rem;font-weight:800;color:var(--psta-dark)}.iteration-secret-link{background:#fff!important;opacity:1!important}.announcement{background:var(--psta-dark)!important}.skip-link{background:var(--psta-dark)!important}.cta-panel{background:var(--cyan-wash)!important;color:var(--ink)!important}.cta-panel h2,.cta-panel p{color:var(--ink)!important}.cta-panel .eyebrow,.cta-panel .text-link{color:var(--psta-dark)!important}.button:hover{background:var(--psta-mid)!important;border-color:var(--psta-mid)!important}@media(max-width:760px){.brand{width:230px;min-height:82px}.brand img{height:66px;max-height:66px}.footer-logo{width:195px}}\n'''
...
def audit(logos):
    fail=[]; logo=ROOT/'assets/img/psta-logo-official.jpg'; footer_logo=ROOT/'assets/img/psta-logo-white.png'
    if not logo.exists() or hashlib.sha256(logo.read_bytes()).hexdigest()!=LOGO_SHA:fail.append('full PSTA logo missing')
    if not footer_logo.exists():fail.append('white PSTA footer logo missing')
    ...
    if not footer_blocks or any(FOOTER_LOGO not in block for block in footer_blocks):fail.append('footer does not use the white PSTA logo')
